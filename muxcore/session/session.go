// Package session provides the Session type representing one downstream client
// connection in the mcp-mux multiplexer.
package session
...
// This is intended for testing the non-buffered write path. For production use,
// prefer NewSession which wraps w in a bufio.Writer for efficiency.
func NewSessionWithRawWriter(id int, w io.Writer) *Session {
	return &Session{
		ID:      id,
		writer:  w,
		notifCh: make(chan []byte, notificationBufferSize),
...
// SetCloser overrides the closer used by Close(). Call this when the reader
// passed to NewSession does not implement io.Closer (e.g. io.MultiReader) but
// an explicit closer (e.g. the underlying net.Conn) is available.
