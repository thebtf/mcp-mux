...
### Ожидание бездействия

Неpersistent shim без запросов и очереди через 10 минут без трафика host
закрывает свою daemon IPC-сессию. Если ещё 30 секунд нет спроса, stable launcher
паркует и engine; следующий host frame запускает active engine, воспроизводит
только кешированный `initialize` и один раз передаёт этот frame. Значения стадий
задают `MCPMUX_SHIM_IDLE_TIMEOUT` и `MCPMUX_SHIM_DORMANT_GRACE` в формате Go duration:
нуль или отрицательное значение отключает соответствующую стадию, неверное
значение оставляет значение по умолчанию. Это не настройки очистки owner или
автозавершения daemon. Owner с `x-mux.persistent: true` не приостанавливается и
не очищается в простое; он работает до явной остановки или завершения daemon.
