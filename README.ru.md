...
2. Буферизует входящие запросы CC (до 1000 сообщений)
3. Отправляет keepalive-уведомления, чтобы предотвратить таймаут CC
...
| Инструмент | Описание |
|-----------|----------|
| `mux_list` | Возвращает все запущенные экземпляры с идентификатором сервера, PID, количеством сессий, ожидающими запросами, классификацией и состоянием кеша. |
...
| `mux_restart` | Останавливает экземпляр и запускает новый daemon-владелец с той же командой. Подключённые сессии переподключаются автоматически при следующем вызове инструмента. |
